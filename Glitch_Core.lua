--[[
  ...
  FROZEN (LO 2026-09-16):
    ...
    - Dropped-egg recovery scans field drops directly from the base
]]
...
local function findDroppedEggGlobal()
	...
	for uid, rec in pairs(recs) do
		if rec.State == "Dropped" then
			...
			-- Recovery must never select a pickup in a player's plot/base.
			-- Only field positions are valid, even when a snapshot says Dropped.
			if not candidate or not isInsideAnyEggField(candidate.Position) then
				continue
			end
			local zoneIndex = 0
